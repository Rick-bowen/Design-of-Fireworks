...
class FountainParticle(Particle):
    def __init__(self,main,x,y,col):
        ...
        #random lifespan
        self.activeDuration = random.choice([3000,3300,3600,3900])
        ...
        if randInt == 1:
            self.posDir = True
        self.XVelOffset = random.randint(0,100)

    def draw(self):
        #select random color and draw
        # choice = random.choice(self.colorCollection)
        pygame.draw.circle(self.main.screen,self.colorCollection,(int(self.position.x),int(self.position.y)),self.size)
