...
class Buttonve:
    ...
    def draw(self):
        ...
        if self.clicked:
            ...
            self.fontColor = self.colour[1]
        pygame.draw.rect(self.surface,self.currentColour,(self.position.x,self.position.y,self.width,self.height))
        #draw text on btn
        if self.text != "":
            font = pygame.font.SysFont('Calibri',28)
            text = font.render(self.text,1,self.fontColor)
            self.surface.blit(text,(self.position.x+50,self.position.y+self.height/2-10))
